...
export class ManagerAssessEmployeeComponent implements OnInit{
  ...
  constructor(private _formBuilder: FormBuilder,private route: ActivatedRoute, private http: HttpClient, private server: HttpServiceService){}
    ...
    if(this.questionForm.valid){
      ...
      for(let i = 0; i < this.userDoneQuestions.length; i++){
        ...
        const answer: Answer = {
          userId: this.empId,
          choiceId: cId,
          managerId: this.systemUser.userId,
      ...
      const url = `${this.server.serverUrl}/managerUpdateChoice?assId=${this.assId}&manId=${this.systemUser.userId}&empId=${this.empId}`
